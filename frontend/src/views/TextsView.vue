    ...
    <div class="texts-layout">
      <section class="texts-section">
        ...
        <v-form class="form-stack" @submit.prevent="onSubmit">
          <v-text-field v-model="name" label="Name" :error-messages="fieldErrors.name" />
          <v-select
            v-model="language"
            ...
            label="Language"
          />
          <v-textarea v-model="content" auto-grow label="Content" min-rows="6" :error-messages="fieldErrors.content" />
          ...
          <div class="action-row">
            ...
            <v-btn color="primary" :disabled="saving" :loading="saving" type="submit" variant="flat">
              Save text
            </v-btn>
          </div>
        </v-form>
      ...
      <section class="texts-section">
        <div class="texts-section__header">
          ...
          <v-btn size="small" variant="text" :disabled="loadingTexts" :loading="loadingTexts" @click="loadTexts">
            Refresh
          </v-btn>
        </div>

        <div v-if="loadingTexts" class="texts-empty">Loading texts...</div>
        <div v-else-if="texts.length === 0" class="texts-empty">No texts yet. Add one to start processing it.</div>
        <div v-else class="texts-list">
          <article v-for="text in texts" :key="text.id" class="text-card">
    ...
    >
      <v-card>
        ...
        <v-card-actions class="delete-dialog__actions">
          <v-spacer />
          <v-btn variant="text" :disabled="deletingTextId !== null" @click="closeDeleteDialog"> Cancel </v-btn>
          <v-btn
            color="error"
...
import type { CreateTextRequest, StudyLanguage, TextItem, TextListResponse, TextResponse } from "../types";
